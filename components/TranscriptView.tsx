...
const TranscriptView: React.FC<TranscriptViewProps> = ({
  ...
  return (
    <div className="flex flex-col h-full bg-white rounded-xl shadow-[0_2px_8px_rgba(0,0,0,0.04)] border border-slate-200 overflow-hidden isolate">
      <div className="flex-none px-3 sm:px-5 py-3 sm:py-4 border-b border-slate-100 bg-white/90 backdrop-blur-md sticky top-0 z-20 space-y-2 sm:space-y-3">
        <div className="flex justify-between items-center">
          <div className="flex items-center gap-2 sm:gap-3">
            <h3 className="font-semibold text-slate-900 text-sm sm:text-base">Transcript</h3>
            <span className="text-[10px] sm:text-xs font-medium text-slate-500 bg-slate-100/50 px-1.5 sm:px-2 py-0.5 rounded-full border border-slate-100">
          ...
          <div className="flex items-center gap-1">
            ...
            <button
              ...
              className="sm:hidden flex items-center justify-center w-8 h-8 text-slate-500 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors"
            ...
            <button
              ...
              className="flex items-center gap-1.5 text-xs font-medium text-slate-500 hover:text-indigo-600 transition-colors px-2 py-1.5 hover:bg-indigo-50 rounded-lg border border-transparent hover:border-indigo-100"
            >
        ...
        {isSearchOpen && (
          <div className="sm:hidden relative animate-fade-in-up">
            <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
            ...
            <input
              ...
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="block w-full pl-9 pr-8 py-2 border border-slate-200 rounded-lg bg-slate-50 text-slate-900 placeholder-slate-400 focus:outline-none focus:bg-white focus:ring-2 focus:ring-indigo-500/10 focus:border-indigo-500 text-sm transition-all"
              placeholder="Find..."
              autoFocus
            />
            {searchQuery && (
        ...
        {topics && topics.length > 0 && (
          <div className="flex flex-wrap gap-1.5 sm:gap-2 pt-1 overflow-x-auto pb-1 -mb-1 scrollbar-hide">
            {topics.map((topic, index) => (
              <button
                ...
                className={`text-[10px] sm:text-[11px] font-medium px-2 sm:px-2.5 py-1 rounded-full border transition-all duration-200 whitespace-nowrap flex-shrink-0
      ...
      <div className="flex-1 overflow-y-auto p-3 sm:p-6 space-y-6 sm:space-y-8 custom-scrollbar bg-slate-50/30">
        {filteredTranscript.map((segment, index) => {
          ...
          return (
            <div
              ...
              className={`flex flex-col max-w-full sm:max-w-[85%] group transition-all duration-200 ease-in-out cursor-pointer hover:opacity-100 ${isProspect ? 'items-start self-start' : 'items-end self-end ml-auto'}`}
            >
              ...
              <div
                className={`px-3 py-2.5 sm:px-5 sm:py-3.5 rounded-2xl text-sm sm:text-[15px] leading-relaxed border shadow-sm transition-all duration-300 ease-out
                  ${isProspect
